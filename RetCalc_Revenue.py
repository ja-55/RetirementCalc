...
def gen_fcst_rev(dt_rng, dt_stt, dt_rtr, dt_ssi,
    ...
    for idx, row in df_mth_sav_rev.loc[dt.date(dt_stt.year + 1,1,1):dt_rtr,:].iterrows():
    ...
    for idx, row in df_mth_sav_rev.loc[dt.date(dt_rtr.year + 1,1,1):dt.date(dt_rtr.year + 1,12,1),:].iterrows():
    ...
    for idx, row in df_mth_sav_rev.loc[dt.date(dt_rtr.year + 2,1,1):dt_ssi,:].iterrows():
        df_mth_sav_rev.loc[idx,'REV_SAL'] = 0
        df_mth_sav_rev.loc[idx,'REV_SSI'] = 0
        df_mth_sav_rev.loc[idx,'REV_GIF'] = 0
        df_mth_sav_rev.loc[idx,'REV_OTH'] = df_mth_sav_rev.shift(12).loc[idx,'REV_OTH']
    ...
    for idx, row in df_mth_sav_rev.loc[dt.date(dt_ssi.year + 1,1,1):dt.date(dt_ssi.year + 1,12,1),:].iterrows():
        df_mth_sav_rev.loc[idx,'REV_SAL'] = 0
    ...
    for idx, row in df_mth_sav_rev.loc[dt.date(dt_ssi.year + 2,1,1):,:].iterrows():
        df_mth_sav_rev.loc[idx,'REV_SAL'] = 0
